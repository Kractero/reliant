...
(async () => {
	...
	detaggingDiv.setAttribute('style', 'background-color: #1F202D; color: #fff; position: fixed; right: 0px; bottom: 0px; z-index: 9999;');
	...
	if (!urlParameters['template-overall']) {
		...
		sidePanel.innerHTML += '<input class="ajaxbutton updatelocalid" type="button" value="Update Localid" data-clicked="0" id="update-localid">';
		...
		topPanel.innerHTML += '<input class="ajaxbutton updatelocalid" type="button" value="Update Localid" data-clicked="0" id="update-localid">';
	...
	async function actionButtonClick(e: MouseEvent): Promise<void> {
		...
		if (value === 'Refresh') {
			...
			for (let i = 0; i !== officerBoxes.length; i++) {
				...
				if (quietLink.innerHTML.indexOf('Founder') !== -1) continue;
				else if (quietLink.innerHTML.indexOf('WA Delegate') !== -1) continue;
				let officerName = delegateRegex.exec(officerBoxes[i].querySelector('.nlink').getAttribute('href'))[1];
			...
			const result = await browser.storage.local.get(['chk', 'roname', 'currentwa']);
			...
			const response = await makeAjaxQuery(`/page=region_control/region=${currentRegionName}`, 'POST', formData);
			const responseElement = document.createRange().createContextualFragment(response);
			const responseInfo = responseElement.querySelector('.info') || responseElement.querySelector('.error');
			if (responseInfo.innerHTML.indexOf('with authority') !== -1) regionStatus.innerHTML = 'Successfully self appointed as RO.';
			else {
			...
			if (response.indexOf('You inform the World Assembly that') !== -1) {
				...
				browser.storage.local.set({ currentwa: '' });
			}
			...
			if (response.indexOf('Welcome to the World Assembly, new member') !== -1) {
				...
				browser.storage.local.set({ currentwa: switchers[0].name });
				...
				updateChk(response);
				switchers.shift();
				browser.storage.local.set({ switchers: switchers });
				actionButton.setAttribute('disabled', '');
			} else if (response.indexOf('Another WA member nation is currently using the same email address') !== -1) regionStatus.innerHTML = `Failed to admit on ${switchers[0].name}. A nation is already in the WA.`;
			else {
	...
	async function refreshEndorseList(e: MouseEvent): Promise<void> {
		...
		if (!urlParameters['template-overall']) currentNation = document.querySelector('#loggedin').getAttribute('data-nname');
		else {
			currentNation = (await browser.storage.local.get('currentwa')).currentwa;
		}
		let response = await makeAjaxQuery(`/page=ajax2/a=reports/view=region.${currentRegionName}/filter=move+member+endo`, 'GET');
		...
		for (let i = 0; i != lis.length; i++) {
			...
			if (canonicalize(nationName) === canonicalize(currentNation)) resigned.push(nationName);
			...
			else if (nationsEndorsed.indexOf(nationName) !== -1) resigned.push(nationName);
	...
	async function manualLocalIdUpdate(e: MouseEvent): Promise<void> {
		...
		browser.storage.local.set({ localid: localId });
	...
	function updateChk(page: string): void {
		...
		if (chk) browser.storage.local.set({ chk: chk.getAttribute('value') });
	}
