...
<div id="container">
	...
	async function updateCurrentSwitcher(e: MouseEvent): Promise<void> {
		...
		browser.storage.local.set({ chk: (responseElement.querySelector('input[name=chk]') as HTMLInputElement).value });
	}

	async function prepButton(e: MouseEvent): Promise<void> {
		...
		if ((e.target as HTMLInputElement).value === 'Apply') {
			...
			const result = await browser.storage.local.get(['localid', 'jumppoint']);
			...
			if (response.indexOf('This request failed a security check.') !== -1) document.querySelector('#status').innerHTML = `Failed to move to ${moveRegion}.`;
			else {
			...
			document.querySelector('#current-switcher-number').innerHTML = String(switchers.indexOf(document.querySelector('#current-switcher').innerHTML) + 1);
			if (typeof nextNation === 'undefined') return;
			document.querySelector('#current-switcher').innerHTML = nextNation;
			let response = await makeAjaxQuery(`/page=un?nation=${nextNation}&password=${result.password}&logging_in=1`, 'GET');
			getChk(response);
	...
	const result = await browser.storage.local.get('prepswitchers');
